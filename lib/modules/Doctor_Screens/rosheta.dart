  ...
  Widget build(BuildContext context) {
    return BlocConsumer<DoctorCubit, DoctorStates>(
      ...
      builder: (BuildContext context, state) {
        vaccineStartDate.text=DateFormat.yMMMd().format(DateTime.now());
        return SafeArea(
            child: Scaffold(
                      ...
                      child: Padding(
                        ...
                        child: Column(children: [
                            Card(elevation: 22.0, clipBehavior: Clip.antiAlias,
                              ...
                              child: Column(
                                children: [Stack(alignment: Alignment.bottomCenter,
                                    children: [
                                      Image(
                                        image: NetworkImage(
                            ...
                            defaultFormField(controller: tashkhess, type: TextInputType.text,
                                validator: (value) {
                                  if (value.isEmpty) {
                                    return 'يجب ادخال البيانات ';
                                  }
                                },
                                label: '   تشخيص المرض', prefixIcon: Icons.medical_services),
                            SizedBox(height: 15,),
                            defaultFormField(
                                controller: vaccineName, type: TextInputType.text,
                                validator: (value) {
                                  if (value.isEmpty) {
                                    return 'يجب ادخال البيانات ';
                                  }
                                },
                                label: '    اسم الدواء أو اللقاح', prefixIcon: Icons.vaccines),
                            SizedBox(height: 15,),
                            defaultFormField(controller: vaccineStartDate, type: TextInputType.text,
                                validator: (value) {
                                  if (value.isEmpty) {
                                    return 'يجب ادخال البيانات ';
                                  }
                            ...
                            SizedBox(height: 15,),
                            BuildDropBottom(dropdownButtonTitle: ' عدد مرات الدواء باليوم',
                              function: (newValue) {
                            ...
                            BuildDropBottom(dropdownButtonTitle: '   مدة الدواء ',
                              function: (newValue) {
                                DoctorCubit.get(context).dropDownButtonNumOfDayWeekMonth(newValue);
                              },
                              items: ['1', '2', '3', '4', '5', '6',].map((valueItem) {
                            ...
                            BuildDropBottom(
                              ...
                              items: ['يوم', 'اسبوع', 'شهر',].map((valueItem) {
                                return DropdownMenuItem(value: valueItem, child: Text(valueItem),);
                              }).toList(),
                                ...
                                function: () {
                                  if(formKey.currentState!.validate()){
                                    ...
                                    DoctorCubit.get(context).sendRosheta(
                                        DiseaseModel(disease: disease, vaccine: vaccine, vaccineDate: vaccineDate,
                                          medicineperDay: medicineperDay, medicineDuraition: medicineDuraition, type: type,)
